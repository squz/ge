...
- **`ge::SafeAreaInsets`** — Direction-agnostic per-edge insets (`y0`, `y1`, `x0`, `x1`, in render-surface pixels) describing the device chrome (camera notch, Dynamic Island, system gestures, home indicator). In ge's SDL/bgfx screen-coord (y-down), `y0` = top, `y1` = bottom, `x0` = left, `x1` = right. Most games consume `Context::drawSafeRect()` / `uiSafeRect()` instead — the rect API is the natural shape. Reach for `Context::drawSafeInsets()` / `uiSafeInsets()` (🎯T37) only when the task is "align flush with a specific chrome edge". All four edges are 0 on platforms with no safe-area concept (desktop) and on wire-mode sessions until the player→server safe-area plumbing lands (🎯T37 follow-up). On iOS / Android, populated from `SDL_GetWindowSafeArea` in `DirectRenderHost`.
