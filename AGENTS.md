...
`AGENTS.md` is the canonical repository instructions, shared across agent
runtimes. `CLAUDE.md` is a one-line `@AGENTS.md` import so Claude Code reads the
...
**Two enforcement sites — both required:**

| Path | How the flag is applied |
|------|-------------------------|
| **Game consumers** (tiltbuggy, multimaze2, …) | `include("${GE_ROOT}/cmake/android-arm64.cmake")` appends `-Wl,-z,max-page-size=16384` to `CMAKE_SHARED_LINKER_FLAGS` before `add_library(main SHARED …)` and before SDL `add_subdirectory()`. |
| **Brokered player** (`tools/android/`) | **Does not** include `android-arm64.cmake` (lean CMake — no libge.a). The **same** `string(APPEND CMAKE_SHARED_LINKER_FLAGS " -Wl,-z,max-page-size=16384")` lives **inline** at the top of `tools/android/app/src/main/cpp/CMakeLists.txt`, **before** `add_subdirectory(SDL)`. Gradle also uses `ANDROID_STL=c++_static` so NDK `libc++_shared.so` is not packaged (that prebuilt is 4 KB under NDK r27). |

Do **not** "simplify" the player CMake by dropping that flag, or the Pixel "ELF alignment check failed" dialog returns (regressed 2026-07-11 and 2026-07-14 when the player path was rebuilt from master without the flag).
...
**Verify** after any Android player or consumer APK build:

```bash
# Player APK (from tools/android/):
...
- **Prebuilt freshness + co-cook:** `make ge/ios`, `ge/ios-device`, `ge/ios-release`, `ge/ios-device-release`, `ge/android`, `ge/android-release` and `ge/android-bundle` run `tools/ensure-prebuilt.sh` for the platform they link (🎯T181.3), right after the `ios/` / `android/` scaffolding guard. Archives are **not committed** (Phase 1 LFS exit), so a fresh clone cooks once and then builds; `GE_SKIP_ENSURE_PREBUILT=1` opts out. Freshness is two separate questions with two separate tools — `tools/verify-manifest.py` (were these archives cooked from the sources on disk now? scripts, submodule SHAs, inputs, NDK pin) and `tools/verify-cook.py` (do the `.a` on disk match `cook.json`?). A third, `tools/verify-prebuilds.py`, answers only "is anyone committing binaries?" and is **not** a freshness gate. It **auto-refreshes with a full co-cook** when stale. Partial `--libge-only` cooks are removed — they desynced libge from vendor libs. **Link-time refusal is platform-symmetric:** Android via `cmake/android-arm64.cmake` → `verify-cook.py`; iOS via the Xcode "Verify prebuilt cook" script phase generated by `tools/ios-build/build_project.rb` (runs every build, even when Xcode is opened without Make). Do not hand-copy archives into `prebuilt/` — the gate will fail.
...
Generic process lives here, not in consumer repos:

- Play Console first session (create app, listing rules, IAP product registration, compliance, org vs personal): [`docs/play-console.md`](docs/play-console.md)
- Upload keystore, signed AAB, Play App Signing: [`docs/android-release.md`](docs/android-release.md)
- License testers: [`docs/iap-testing.md`](docs/iap-testing.md)

`make ge/android-bundle` produces the signed AAB. The upload private key lives in the macOS keychain (service `play-upload`); ge does not choose the keychain *account* — that is the organisation slug (`ANDROID_KEYSTORE_KEYCHAIN_ACCOUNT` / game `STUDIO`). Do not keep a persistent `.jks` on disk. The consuming **game** keeps package id, Console app id, listing copy, SKUs, and store assets. Catalogue, pricing, and review waits are human-owned — do not invent SKUs or submit store metadata without explicit approval.
