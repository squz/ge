#!/usr/bin/env bash
# Prebuild vendor static libs for iOS arm64 → ge/prebuilt/ios-arm64/.
#
# Run from the ge repo root. Requires all submodules to be initialized
# (git submodule update --init --recursive). Outputs nine static libs:
#   bgfx, bx, bimg, box2d, lunasvg_ge, plutovg_ge, sqlite3_ge, lz4_ge, liteparser.
#
...
# recompiling vendor sources, cutting per-CI-run compile time by ~60s.
#
# Refresh workflow: bump submodule SHAs, then re-run this script + lift-headers.sh,
# then commit. See docs/vendor-prebuilds.md.
#
