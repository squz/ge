#!/usr/bin/env python3
"""Regression tests for tools/verify-manifest.py (🎯T78, 🎯T181.3).
...
These tests used to target verify-prebuilds.py, which owned the
staleness question until the Phase 1 LFS exit repurposed that name for
"are binaries tracked in git?". The staleness oracle now lives in
verify-manifest.py and the tests moved with it; the tracked-binaries
contract has its own suite in test_verify_prebuilds.py.

Run: `python3 tools/test_verify_manifest.py`
...
VERIFIER = Path(__file__).resolve().parent / "verify-manifest.py"
...
class VerifierTests(unittest.TestCase):
    ...
    def test_happy_path_passes(self) -> None:
        ...
        self.assertIn("manifest ok", r.stdout)
    ...
    # Archive hashes are deliberately NOT checked here. Since the Phase 1
    # LFS exit the .a files are a local cache, so a committed manifest can
    # never match another machine's cook; integrity is verify-cook.py's job
    # (archives vs the cook.json written beside them). This file only asks
    # whether the *sources* still match what was cooked.
